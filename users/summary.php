          ...
          <div>
            ...
            <div>
              <div class="titlefield">
                <span>Sales and Expenses Monitoring</span>
              </div>
            </div>
              ...
              <table>
                ...
                <tr>
                <td class="formfield"><label>Number of sacks: </label></td>
                <td class="formfield"><input class="memf" type="number" name="sack" min="1" required=""></td>
                </tr>
                <tr>
                <td class="formfield"><label>Sack weight: </label></td>
                <td class="formfield"><input class="memf" type="text" name="weight" ></td>
                </tr>
              
                <tr>
                <td class="formfield"><label>Price per sack: </label></td>
                <td class="formfield"><input class="memf" type="text" name="price" ></td>
                </tr>
                
                <tr>
                <td class="formfield"><label>Total price: </label></td>
                <td class="formfield"><input class="memf" type="text" name="total" ></td>
                </tr>
...
<div class="main-panel">
  ...
  <div class="smain" style="background-color: white;">
    ...
    <?php
    $sql = "SELECT c.cropid, f.farm_number, c.cropping, DATE_FORMAT(c.harvest,'%Y') FROM cropp c, farmtbl f WHERE c.farm = f.farmid AND c.stat =0 AND c.uzer = $user ORDER by f.farmname, c.harvest desc";
    $res = setQuery($sql);
  ...
  <button class="btn" onclick="printR()" style="font-size: 30px;">🖨️</button>
            </div>
          </div>
          <div style="display: flex; float: right;padding-bottom: 10px; background-color: white;">
            <?php
              if($_SESSION['userole']!=2){
                ?>
                  <button style="display: none;" onclick="document.getElementById('modals').style.display='';" class="ybtn main-add no-border">Add sales</button>
                <?php
              }
              else{
...
<script type="text/javascript">
  ...
  function search(){
    var val = document.getElementById('cbcrops').value;
    var userid = <?php echo $_SESSION['userid']; ?>;
    var xml = new XMLHttpRequest();
    xml.onload = function(){
      document.getElementById('inserhere').innerHTML = this.responseText;
    }
    xml.open("GET","summarytbl.php?value="+val+"&userid="+userid);
    xml.send();
  }
...
<script type="text/javascript">
      ...
      mywindow.document.write('<link href="printstyle.css" rel="stylesheet" />');
      var data = document.getElementById('inserhere').innerHTML;
