            ...
            <div>
              ...
              <table>
                <form method="post" action="actions/admem.php?action=1">
                  <input type="hidden" name="" id="i00" value="<?php echo $user; ?>">
                <tr>
                <td class="formfield"><label>Crop: </label></td>
                <td class="formfield"><?php crops($user,'memf','i1'); ?></td>
                </tr>
                <tr>
                <td class="formfield"><label>Total production: </label></td>
                <td class="formfield"><input min="0" class="memf dis" type="number" name="i0" id="i0" readonly="" required=""></td>
                </tr>
                <tr>
                <td class="formfield"><label>Partida sa ani: </label></td>
                <td class="formfield"><input min="0" class="memf" onchange="getTotal()" onkeyup="getTotal()" type="number" id="i2" name="i2" required="" value="0"></td>
                </tr>
                <tr>
                <td class="formfield"><label>Partida sa Triser: </label></td>
                <td class="formfield"><input min="0" class="memf" type="number" id="i3" onchange="getTotal()" onkeyup="getTotal()" name="i3" required="" value="0"></td>
                </tr>
                <tr>
                <td class="formfield"><label>Partida sa porsyentador: </label></td>
                <td class="formfield"><input min="0" class="memf" type="number" onchange="getTotal()" onkeyup="getTotal()" id="i4" name="i4" required="" value="0"></td>
                </tr>
                <tr>
                <td class="formfield"><label>Kunsumo: </label></td>
                <td class="formfield"><input class="memf" type="number" onchange="getTotal()" onkeyup="getTotal()" id="i5" name="i5" required="" min="0" value="0"></td>
                </tr>
                <tr>
                <td class="formfield"><label>Total (Must be the same as production): </label></td>
                <td class="formfield"><input required="" onkeypress="return false;" class="memf dis" type="number" name="i6" id="tt" value="0"></td>
                </tr>
                
               
                <tr>
                  <td colspan="2" align="right" class="formfield pad5"><button name="ii" type="submit">Save</button></td>
                </tr>
              </form>
              </table>
      ...
      <div>
        ...
        <div class="stitle">
          ...
          <div style="float: left;">
            <div>
              <input style="display: none;" type="search" name="" class="searchfield" id="search" placeholder="Search Inventory here..."  oninput="search()">
            </div>
          </div>
          <div style="display: flex; float: right;padding-bottom: 10px; background-color: white;">
            <?php
              if($_SESSION['userole']!=2){
                ?>
                  <button onclick="document.getElementById('modals').style.display='';" class="ybtn main-add no-border">Inventory</button>
                <?php
              }
              else{
        ...
        <div id="inserhere">
           ...
           <div>
             <a href="actions/retrieve.php?link=inventory.php&table=inventory&column=user" onclick="return confirm('are you sure to proceed?')">Retrieved deleted data</a>
           </div>
          </div>
        </div>
      </div>
...
<script type="text/javascript">
  ...
  function search(){
    ...
    xml.open("GET","inventorytbl.php?value="+val+"&userid="+userid);
    xml.send();
  }
...
window.addEventListener("load",showProductions);
...
function showProductions(){
  ...
  var uzer = document.getElementById('i00').value;
  var xml = new XMLHttpRequest();
  xml.onload = function(){
    document.getElementById('i0').value = this.responseText;
    document.getElementById('tt').max = this.responseText;
    document.getElementById('tt').min = this.responseText;
  }
  xml.open("GET","actions/calculate.php?q="+val+"&w="+uzer);
  xml.send();
}
